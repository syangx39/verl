#!/usr/bin/env bash
#SBATCH --job-name=magellan-prefill16k-8n32g
#SBATCH --nodes=8
#SBATCH --gres=gpu:4
...
# =============================================================================
# Magellan prefill-heavy multi-node run: 8 nodes x 4 GPU = 32 GPUs.
...
# be visible to both.
#
# Check afterwards (vs magellan 1n4g / 4n16g):
...
srun -N1 -n1 -w "$head_node" --overlap --mem=200G \
  ...
  ray status
  NNODES=8 TOTAL_STEPS=\${TOTAL_STEPS:-20} bash $MAGELLAN_DIR/run_meta_prefill_16k.sh \
    trainer.test_freq=-1 \
